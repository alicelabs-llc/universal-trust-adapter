...
export class RedisNonceStore implements NonceStore {
  ...
  constructor(opts: { url: string; token: string; keyPrefix?: string }) {
    this.redisUrl = opts.url;
    this.redisToken = opts.token;
    this.keyPrefix = opts.keyPrefix || 'uta:nonce:';
  }

  private async redis(command: string[], opts: { ttl?: number } = {}): Promise<any> {
    const path = opts.ttl
  ...
  async retrieve(nonce: string): Promise<StoredChallenge | null> {
    ...
    return JSON.parse(data.result) as StoredChallenge;
  }
...
export class PoPManager {
  ...
  constructor(store: NonceStore, opts: { challengeTtlMs?: number } = {}) {
    this.store = store;
   ...
   */
  async issueChallenge(credentialId: string, audience: string): Promise<StoredChallenge> {
