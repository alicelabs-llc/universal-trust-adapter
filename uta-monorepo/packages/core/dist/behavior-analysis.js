...
exports.BEHAVIOR_ANALYSIS_VERSION = exports.PATTERNS = void 0;
 ...
 */
function analyzeFilesystem(accesses) {
    const flags = [];
 ...
 */
function analyzeProcesses(spawns) {
    const flags = [];
 ...
 */
function analyzeBehavior(networkConnections, fileAccesses, processSpawns, params) {
    ...
    let riskScore = 0;
    if (criticalCount > 0)
        riskScore = Math.min(10, 5 + criticalCount * 2);
    else if (warnCount > 0)
        riskScore = Math.min(5, warnCount);
...
exports.BEHAVIOR_ANALYSIS_VERSION = '1.0.0';
