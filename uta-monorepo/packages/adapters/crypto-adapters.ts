...
import { canonicalize, canonicalHash, verify as ed25519Verify, DOMAINS } from '../core/crypto.js';

// ============================================================================
// P1-2: OAuth/OIDC — Real JWT Verification
// ============================================================================
 ...
 */
export function verifyJWT(jwt: string, publicKeyPem: string): JWTVerifyResult {
  const issues: string[] = [];
  ...
  try {
    switch (header.alg) {
      case 'RS256':
        signatureValid = crypto.verify('RSA-SHA256', signingInput, publicKeyPem, signature);
        break;
      ...
      case 'ES256':
        ...
        signatureValid = crypto.verify('SHA256', signingInput, {
          key: publicKeyPem,
          dsaEncoding: 'der',
        }, derSignature);
        break;
 ...
 */
function rawEcdsaToDer(rawSig: Buffer): Buffer {
  if (rawSig.length !== 64) {
 ...
 */
export function verifyW3CVC(vc: Record<string, unknown>, publicKeyPem: string): VCVerifyResult {
  const issues: string[] = [];
  ...
  if (vc['expirationDate']) {
    if (new Date(vc['expirationDate'] as string) < new Date()) {
      issues.push(`expired: ${vc['expirationDate']}`);
    }
  }

  // 7. Extract issuer and subject
  const issuer = typeof vc['issuer'] === 'string' ? vc['issuer'] as string :
    (vc['issuer'] as Record<string, unknown>)?.id as string || 'unknown';
  const subject = (vc['credentialSubject'] as Record<string, unknown>)?.id as string || 'unknown';
