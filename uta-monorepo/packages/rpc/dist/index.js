...
exports.TrustServiceClient = void 0;
...
function createRPCServer(config) {
    const revocationStore = config.revocationStore || new revocation_js_1.InMemoryRevocationStore();
    const receiptStore = new receipts_js_1.ReceiptStore();
    const receiptGen = new receipts_js_1.ReceiptGenerator(receiptStore, config.gatewayKeyPair);
    const gateway = new index_js_1.TrustGateway({
    ...
    let server = null;
    const decisionSubscribers = [];
    async function handleRPC(method, body) {
        ...
        try {
            ...
            if (method === 'IssueW3CVC') {
                ...
                const signed = (0, crypto_adapters_js_1.issueW3CVC)(cred, config.caKeyPair.privateKeyPem);
    ...
    async function verifyAnyFormat(cred, caKey, opts) {
        if (cred.jwt) {
            const r = (0, crypto_adapters_js_1.verifyJWT)(cred.jwt, caKey);
            return { valid: r.valid, format: 'jwt', issues: r.issues, issuer: r.issuer, subject: r.subject };
        }
        if (cred.atc_version?.startsWith?.('3.')) {
            const r = (0, atc_v3_js_1.verifyATCv3)(cred, caKey);
        ...
        if (cred['@context']?.includes?.('https://www.w3.org/2018/credentials/v1')) {
            const r = (0, crypto_adapters_js_1.verifyW3CVC)(cred, caKey);
    ...
    return {
        listen: (port, host = '0.0.0.0') => {
            return new Promise((resolve) => {
                server = node_http_1.default.createServer(httpHandler);
...
class TrustServiceClient {
    ...
    async revokeCredential(credentialId, reason, adminApiKey) {
...
exports.TrustServiceClient = TrustServiceClient;
