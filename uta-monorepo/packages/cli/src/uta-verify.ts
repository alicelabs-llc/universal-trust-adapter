...
interface CliArgs {
  ...
  jwt?: string;
  format?: 'atc-v3' | 'vc' | 'a2a' | 'eat' | 'zta' | 'mcp';
  caKeyPath?: string;
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 2; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--ca-key') { args.caKeyPath = argv[++i]; continue; }
    if (a === '--registry-key') { args.registryKeyPath = argv[++i]; continue; }
    if (a === '--jwt') { args.jwt = argv[++i]; continue; }
    if (a === '--atc-v3') { args.format = 'atc-v3'; continue; }
    if (a === '--vc') { args.format = 'vc'; continue; }
    if (a === '--a2a') { args.format = 'a2a'; continue; }
    if (a === '--eat') { args.format = 'eat'; continue; }
    if (a === '--zta') { args.format = 'zta'; continue; }
    if (a === '--mcp') { args.format = 'mcp'; continue; }
    if (a === '--json') { args.json = true; continue; }
    if (a === '--verbose' || a === '-v') { args.verbose = true; continue; }
    if (a === '--allow-expired') { args.allowExpired = true; continue; }
    if (a === '--version' || a === '-V') { args.showVersion = true; continue; }
    if (a === '--help' || a === '-h') {
      printHelp();
      process.exit(0);
    }
  ...
  if (positional.length > 0) args.credentialFile = positional[0];
  return args;
}
...
async function main() {
  ...
  if (args.jwt) {
    ...
    try {
      const content = fs.readFileSync(args.credentialFile, 'utf-8');
      const parsed = JSON.parse(content);
      // If the file is a UTA test vector (has 'input' field), unwrap it.
      // This makes it easy to test the CLI against the committed vectors.
      credential = parsed.input || parsed;
    } catch (e) {
      console.error(`Error reading ${args.credentialFile}: ${(e as Error).message}`);
      process.exit(2);
    }
  }

  // Load CA key
  let caPublicKeyPem: string | undefined;
  if (args.caKeyPath) {
    try {
      caPublicKeyPem = fs.readFileSync(args.caKeyPath, 'utf-8');
    } catch (e) {
      console.error(`Error reading CA key ${args.caKeyPath}: ${(e as Error).message}`);
      process.exit(2);
    }
  }

  let registryPublicKeyPem: string | undefined;
  if (args.registryKeyPath) {
    try {
      registryPublicKeyPem = fs.readFileSync(args.registryKeyPath, 'utf-8');
    } catch (e) {
      console.error(`Error reading registry key ${args.registryKeyPath}: ${(e as Error).message}`);
      process.exit(2);
    }
  }
...
function autoDetectFormat(cred: any): string | undefined {
  if (cred.jwt) return 'jwt';
  if (cred.atc_version?.startsWith('3.')) return 'atc-v3';
  if (cred['@context']?.includes?.('https://www.w3.org/2018/credentials/v1')) return 'vc';
  if (cred.agentCard || (cred.name && cred.url && cred.capabilities)) return 'a2a';
  if (cred.payload && cred.signature && cred.alg) return 'eat';
  if (cred.agent_id && cred.identity && cred.trust && cred.signature) return 'zta';
  if (cred.name && cred.tools && cred.transport) return 'mcp';
  return undefined;
}

async function verifyCredential(cred: any, format: string, caKey?: string, registryKey?: string, skipExpiry: boolean = false): Promise<any> {
  ...
  function canonicalize(value: any): string {
    ...
    if (t === 'number') {
      if (!Number.isFinite(value)) throw new Error(`JCS: ${value}`);
  ...
  function ed25519Verify(payload: any, signatureHex: string, publicKeyPem: string, domain: string): boolean {
    try {
  ...
  const result: any = { format, valid: false, issues: [] };
  ...
  if (format === 'atc-v3') {
    ...
    if (!sigValid) result.issues.push('Ed25519 signature verification failed');

    const canonical = canonicalize(payload);
    const expectedEvidenceHash = 'sha256:' + crypto.createHash('sha256').update(canonical + sig.value, 'utf-8').digest('hex');
    if (sig.evidence_hash !== expectedEvidenceHash) result.issues.push('evidence_hash mismatch');
    ...
    if (cred.lifecycle?.revoked) result.issues.push('revoked (inline)');
    ...
    let sigValid = false;
    try { sigValid = crypto.verify(null, signingInput, publicKey, signature); }
    catch { sigValid = false; }
    if (!sigValid) result.issues.push('Ed25519Signature2020 verification failed');
    ...
    if (header.alg === 'none') { result.issues.push('alg=none forbidden'); return result; }
    if (header.alg === 'HS256') { result.issues.push('HS256 not supported'); return result; }
    ...
    try {
      const publicKey = crypto.createPublicKey(caKey!);
      if (header.alg === 'RS256') sigValid = crypto.verify('RSA-SHA256', signingInput, publicKey, signature);
      else if (header.alg === 'EdDSA') sigValid = crypto.verify(null, signingInput, publicKey, signature);
      else if (header.alg === 'ES256') sigValid = crypto.verify('SHA256', signingInput, { key: publicKey, dsaEncoding: 'ieee-p1363' } as any, signature);
      else { result.issues.push(`unsupported alg: ${header.alg}`); return result; }
    } catch (e) { result.issues.push(`verify error: ${(e as Error).message}`); }
    if (!sigValid) result.issues.push(`${header.alg} signature verification failed`);
    ...
    if (!card.proof) { result.issues.push('missing proof'); return result; }
    if (card.proof.type !== 'Ed25519Signature2020') { result.issues.push(`unsupported proof type: ${card.proof.type}`); return result; }
    const signature = Buffer.from(card.proof.proofValue, 'base64url');
    if (signature.length !== 64) { result.issues.push(`signature wrong length: ${signature.length}`); return result; }
    ...
    if (!sigValid) result.issues.push('A2A signature verification failed');
    ...
    if (!cred.signature) { result.issues.push('missing signature'); return result; }
    ...
    let sigValid = false;
    try {
      if (cred.alg === 'EdDSA') sigValid = crypto.verify(null, signingInput, publicKey, sigBytes);
      else if (cred.alg === 'ES256') sigValid = crypto.verify('SHA256', signingInput, { key: publicKey, dsaEncoding: 'ieee-p1363' } as any, sigBytes);
      else if (cred.alg === 'RS256') sigValid = crypto.verify('RSA-SHA256', signingInput, publicKey, sigBytes);
      else { result.issues.push(`unsupported alg: ${cred.alg}`); return result; }
    } catch (e) { result.issues.push(`verify error: ${(e as Error).message}`); }
    if (!sigValid) result.issues.push(`${cred.alg} signature verification failed`);
    ...
    if (!cred.signature) { result.issues.push('missing signature'); return result; }
    if (cred.signature.domain !== 'UTA-ZTA-CARD') { result.issues.push(`wrong domain: ${cred.signature.domain}`); return result; }
    const { signature, ...payload } = cred;
    const sigValid = ed25519Verify(payload, cred.signature.value, caKey!, 'UTA-ZTA-CARD');
    if (!sigValid) result.issues.push('ZTA signature verification failed');
    ...
    if (cred.signature.domain !== 'UTA-MCP-CARD') { result.issues.push(`wrong domain: ${cred.signature.domain}`); return result; }
    const { signature, ...payload } = cred;
    const sigValid = ed25519Verify(payload, cred.signature.value, registryKey!, 'UTA-MCP-CARD');
    if (!sigValid) result.issues.push('MCP signature verification failed');
    else result.trust_score = 5;
...
function printHumanReadable(result: any, verbose: boolean): void {
  const status = result.valid ? '✅ VALID' : '❌ INVALID';
  console.log(`${status}  [format=${result.format}]`);
  if (result.credential_id) console.log(`  credential_id: ${result.credential_id}`);
  if (result.issuer) console.log(`  issuer:       ${result.issuer}`);
  if (result.subject) console.log(`  subject:      ${result.subject}`);
  if (result.expires_at) console.log(`  expires_at:   ${result.expires_at}`);
  if (result.trust_score !== undefined) console.log(`  trust_score:  ${result.trust_score}`);
