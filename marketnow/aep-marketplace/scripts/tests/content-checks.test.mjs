...
test('R-05: stats.json exists and has correct structure (v5.5 catalog)', () => {
  assert.ok(STATS_JSON.discovery.total_mcp_servers === 69077, 'total_mcp_servers should be 69077 (L1 index-certified)');
  assert.ok(STATS_JSON.security.l1_index_certified === 69077, 'l1_index_certified should be 69077');
...
test('R-10: data layer has no stale 9,248 counts (agent-ping feeds the homepage)', () => {
  ...
  assert.ok(PING_JSON.stats.total_skills === 69077, 'agent-ping total_skills should be 69077');
